...
export class MyRequestService {
    ...
    // Observable for getting created requests 
    getCreatedRequestsObservable : Observable<any>
    // Observable for getting request details
    getRequestDetailsObservable : Observable<any>
    constructor(
        ...
        this.getCreatedRequestsObservable = Observable.create(observer => {
            /*
            ...
            */
            this.presentLoading();
            this.memoryService.updatePostObservers('getCreatedRequestsObservable', true);
            ...
            this.http.post(APP_CONFIG.baseUrl+'User/GetServiceRequestDetails', this.postData, {headers: this.headersInstance}).subscribe(data => {
                // to remove the refernce string from memory
                this.memoryService.updatePostObservers('getCreatedRequestsObservable', false);
                this.loading.dismiss();
                // call next methd and pass data to the component
                observer.next(data);
                observer.complete();
            }, ()=>{
        ...
        // observable for POST api
        this.getRequestDetailsObservable = Observable.create(observer => {
            /*
                before calling the POST api, push the refernce string to memory.
                refer the updatePostObservers method in MemoryService 
                refer registerBackButtonAction in app.component.ts
            */
            this.presentLoading();
            this.memoryService.updatePostObservers('getRequestDetailsObservable', true);
            // before calling the POST api, this.postData is set by calling the setPostData method of this same service from the component
            this.http.post(APP_CONFIG.baseUrl+'User/GetServiceRequestDetails', this.postData, {headers: this.headersInstance}).subscribe(data => {
                // to remove the refernce string from memory
                this.memoryService.updatePostObservers('getRequestDetailsObservable', false);
                this.loading.dismiss();
                // call next methd and pass data to the component
                observer.next(data);
                observer.complete();
